...
export default function Page(){
  const [user,setUser]=useState<User|null>(null),[initialState,setInitialState]=useState<any>(null),[ready,setReady]=useState(false),[error,setError]=useState(""),[sending,setSending]=useState(false),[showPassword,setShowPassword]=useState(false);
  ...
  if(!user)return <main className="login-page"><form className="login-card" onSubmit={login}><img src="/logo-monocenter.jpg" alt="Monocenter Alinhamento Técnico"/><div><small>SISTEMA INTERNO</small><h1>Acesso Monocenter</h1><p>Entre com seu usuário e senha para acessar a agenda e os serviços.</p></div><label>Usuário<input name="username" autoComplete="username" autoCapitalize="none" required/></label><label>Senha<span className="password-input-wrap"><input name="password" type={showPassword?"text":"password"} autoComplete="current-password" required/><button type="button" className="password-toggle" onClick={()=>setShowPassword((value)=>!value)} aria-label={showPassword?"Ocultar senha":"Mostrar senha"} aria-pressed={showPassword}>{showPassword?"Ocultar":"Ver senha"}</button></span></label>{error&&<p className="login-error" role="alert">{error}</p>}<button className="primary" disabled={sending}>{sending?"Entrando…":"Entrar no sistema"}</button><small className="login-help">Em caso de dificuldade, fale com a Gestão.</small></form></main>;
